# Strata — Architecture v5 (FINAL)

**Status:** Approved for implementation (v5.0 FINAL, closes Critic A v4 last finding `A-N1-v4` + paper-asymptote acknowledgment)
**Date:** 2026-04-28
**Author:** Architect role
**Audience:** experienced Rust engineers; both critics returning for final grade; implementing team Monday morning
**Working name:** **Strata** (project brand) — published crates use the `strata-*` prefix (see §0.1)
**Internal version:** v5.0 FINAL (was v4.0; bumped to close `A-N1-v4` output-size DoS + parallel limit tightening + B-asymptote note)
...
ATC-BRG-5 (v5: total_bytes_cap; A-N1-v4 tightening)
  ...
  WHEN   strata-signing::verify_bridge(biscuit, verifier_node_id) runs
  THEN   it returns Err(STRATA-E0122)
  TEST   crates/strata-signing/tests/bridge_attenuation.rs::missing_total_bytes_cap_rejected
...
ATC-BRG-6 (v5: cumulative byte enforcement)
  ...
  TEST   crates/strata-plugin-mesh-bridge/tests/total_bytes_cap.rs
```
...
ATC-INT-5 (output-size DoS short-circuit, INV-INT-2; A-N1-v4)
  ...
  TEST   crates/strata-plugin-scheduler/tests/integrity/oversized_output_short_circuits.rs
...
ATC-INT-6 (StreamingTask total-byte cap; A-N1-v4 tightening)
  ...
  TEST   crates/strata-plugin-scheduler/tests/stream_total_bytes.rs
```
