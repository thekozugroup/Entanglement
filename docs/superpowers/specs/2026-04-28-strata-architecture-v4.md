# Strata — Architecture v4

**Status:** Proposed (v4.0, addresses Critic A 95/100 and Critic B 95/100 reviews of v3)
**Date:** 2026-04-28
...
**Internal version:** v4.0 (was v3.0; bumped to absorb both v3 critic re-reviews — minor findings + provable-on-paper closure)
...
**Project provenance — `strata-dev` GitHub org (Critic B v3 `B-strata-dev-org-provenance`).** The canonical home of the project is the **`strata-dev` GitHub org**, reserved at v4 publication. The org owns:

| Surface | Owner | Purpose |
|---|---|---|
| `github.com/strata-dev/strata` | strata-dev | Workspace monorepo (mirror of thekozugroup/strata for compatibility; canonical going forward). |
| `github.com/strata-dev/homebrew-tap` (alias `strata-dev/tap`) | strata-dev | Homebrew formulae for `strata`, `cargo-strata`. |
| `strata-dev.github.io` | strata-dev | Documentation site (mirror of `docs.strata.dev`). |
| crates.io ownership of `strata-runtime`, `strata-core`, `strata-broker`, `strata-manifest`, `strata-signing`, `strata-oci`, `strata-host`, `strata-ipc`, `strata-types`, `strata-wit`, `strata-https-fetch`, `strata-sdk`, `strata-i18n`, `cargo-strata`, and every `strata-plugin-*` crate listed in §10 | strata-dev publisher group | First-party crate index. |
| sigstore/cosign signing identity `strata-dev@strata.dev` (OIDC-bound to the org) | strata-dev | Cosign signature alongside Ed25519 release sig. |
...
2. **Sigstore/cosign** signature using OIDC against the `strata-dev` GitHub org. This is **defense in depth**: even if the Ed25519 key were silently swapped, the cosign provenance ties the artifact to a specific org-owned GitHub Actions run. Daemon optionally verifies cosign via `[trust] require_cosign = true` (off by default for hobbyist UX, on by default for `max_tier_allowed <= 3` deployments).

Cosign verification implementation lives in `strata-signing`; the verifier inspects the Rekor transparency-log entry to detect post-hoc backfilled signatures.
...
// (d) Mandatory marker — recipients reject if absent
...
**Test vectors.** The `strata-signing` crate ships three test vectors at `crates/strata-signing/testdata/bridge-vectors/`:

| Vector | Expected outcome | Why |
|---|---|---|
| `valid-bridge.bsk` | accept | All three attenuations present; chain signed by endpoint_A; bridge NodeId pinned to endpoint_B. |
| `missing-rate-limit.bsk` | reject (`STRATA-E0118`) | (b) absent → bridge could amplify endpoint's bandwidth caps. |
| `wrong-dest.bsk` | reject (`STRATA-E0119`) | (a) pins a NodeId other than the verifier's; bridge attempted lateral delegation. |
| `expired-ttl.bsk` | reject (`STRATA-E0120`) | (c) `time($t)` exceeds biscuit's wall-clock at verification. |
| `no-bridge-marker.bsk` | reject (`STRATA-E0121`) | (d) `bridge_cap(true)` missing — receiver assumes a non-bridge cap is being abused as a bridge cap. |

CI runs the verifier against every vector; a regression that accepts any rejection vector breaks the build.

**Implementation invariant (one line, machine-checkable).** `Bridge biscuits MUST satisfy: |attenuation_facts ∩ {dest_pin, rate_limit, ttl_le_3600s, bridge_marker}| == 4` — enforced in `strata-signing::verify_bridge` with a property test in `crates/strata-signing/tests/bridge_attenuation.rs`.
...
**StalledEvent surface.** Operators see stalled streams in `strata logs scheduler --filter StalledEvent` and as the Prometheus counter `strata_scheduler_streams_stalled_total{peer_id="..."}`.
...
**Rule 1 — Curated stdlib metrics signed by Strata's publisher key.** Strata ships a fixed, audited set of stdlib metrics signed by `ed25519:strata-stdlib-metrics-2026` (the project's publisher key, distinct from any release key). These run in the broker's wasm sandbox at **tier 2** (no network, no fs):

| Metric crate | Computes | When to use |
|---|---|---|
| `strata-metric-bleu-4` | BLEU-4 over token streams | Generic NLG eval. |
| `strata-metric-embedding-cosine` | Cosine over a pinned sentence-embedding model | Semantic equivalence of free-text outputs. |
| `strata-metric-numerical-l2-relative` | Relative L2 over numeric tensors | Numerical workloads (matrix multiply, simulation). |
| `strata-metric-image-ssim` | SSIM over images | Vision pipelines; fixed reference impl. |
...
**Sequence diagram explicitly added to §7.5.** The above is canonical. If a paper-protocol divergence ever ships, the v4 invariant is the spec. Implementation lives in `strata-plugin-scheduler::integrity::semantic_equivalent` with a verifier in `strata-plugin-scheduler::tests::integrity::no_worker_metric_execution` that asserts (via wasmtime trace) that no metric component is ever instantiated on the worker side.
...
Enforced in `strata-plugin-scheduler::work_unit::verify_envelope` and tested by `tests::policy_immutability::cannot_mutate_after_dispatch`.
...
**Wrapper-bypass UX trap — opt-in shell shim (Critic A v3 `A-N4-v3`).** "User thinks they're sandboxed but isn't" is a real footgun. v4 mitigates with an **opt-in shell function/alias** installed at `strata install`-time:

1. **Default install** adds (idempotently, with a clear marker) to `~/.zshrc` / `~/.bashrc` / `~/.config/fish/conf.d/strata.fish`:

   ```bash
   # >>> strata wrapper >>>
   # Installed by `strata install` on 2026-04-28. Manage via `strata wrapper {enable,disable,status}`.
   claude-code() {
     if [ -z "$STRATA_SESSION_ID" ]; then
       printf '\033[33mwarning:\033[0m claude-code launched outside Strata session.\n' >&2
       printf '         Tools are NOT routed through Strata gateway.\n' >&2
       printf '         For sandboxed mode: strata agent run claude-code "$@"\n' >&2
       printf '         To silence: strata wrapper disable\n' >&2
     fi
     command claude-code "$@"
   }
   # <<< strata wrapper <<<
...
2. **Opt-in.** The first-run wizard asks: *"Install shell wrappers that warn when agent hosts run outside Strata? [Y/n]"*. User can disable at any time via `strata wrapper disable` (which removes the block) or by setting `[wrappers] install = false` in `~/.strata/config.toml`. Users who never want the shim can run `strata install --no-wrapper`.

3. **Doctor check.** `strata doctor` validates the wrapper is in place AND not shadowed:
   - If the wrapper block is missing from the user's rc files → warning with `strata wrapper enable`.
   - If `which claude-code` resolves to a binary path before the shell function (i.e. the function is shadowed by `PATH` or a different alias) → warning naming the shadow source: `warning: 'claude-code' is shadowed by /usr/local/bin/claude-code (PATH ahead of shell function). Run 'strata wrapper repair' or 'unalias claude-code'.`
   - If `STRATA_SESSION_ID` is set but the user's shell history shows recent `claude-code` invocations without the wrapper → informational note.

4. **Wrapper opt-out is loud.** Disabling via `strata wrapper disable` records the choice in `~/.strata/config.toml`; subsequent installs respect the choice. There is no way to silently lose the wrapper.
...
**Reproducible packaging (Critic B v3 `B-reproducible-packaging`).** `cargo strata package` MUST produce a **byte-identical** `.tar.zst` from the same source on the same toolchain. This makes signature verification meaningful: a verifier (or a third-party rebuilder) can rebuild from source and confirm the bytes match what was signed. Implementation requirements:

| Determinism input | Mechanism |
|---|---|
| Build timestamps in `.wasm` | `SOURCE_DATE_EPOCH` honored by `cargo-component`/`wasm-tools`; `cargo strata package` exports `SOURCE_DATE_EPOCH=$(git -C . log -1 --pretty=%ct)` automatically (override with `--source-date-epoch`). |
...
| Cargo lockfile | `Cargo.lock` MUST be present and committed; `cargo strata package` refuses to package without it (`error: Cargo.lock missing — non-reproducible build`). |
| WIT versions | Every WIT package import in `world.wit` MUST pin to a `@x.y.z` version (no `@x` floats). `cargo strata package` validates and rejects floating imports. |
...
1. `strata config reload` parses the new config and computes the **delta set** = `{p ∈ loaded_plugins : p.tier > new_max_tier_allowed}`.
...
4. **Default = unload-and-disable.** On `u`: each plugin in the delta set is gracefully drained (current invocations complete with `task_timeout`), then unloaded; the plugin's enabled flag is set to `false` in `config.toml` so it does not resurface on next start. The daemon emits a structured `STRATA-W0214` warning per plugin and a final `STRATA-I0215` summary.

5. **Cancel.** On `c`: the proposed `max_tier_allowed` change is reverted in `config.toml` (with operator's permission to back out the edit) and the daemon continues with the previous ceiling. A `STRATA-I0216` log entry records the cancelled lowering.

6. **Scripted use.** `strata security set-max-tier <N> --force-unload` is the non-interactive path: it auto-selects unload-and-disable for any plugins above `N`. **NEVER silent** — even with `--force-unload`, the operation prints the affected-plugin list to stderr and writes the audit-log entry. There is no flag that skips the audit trail.

7. **Validation hook.** `strata config validate` (run on `reload` and standalone) computes the delta set without applying changes. CI uses `validate` to fail-fast on configs that would orphan plugins.
...
**Phase-1 falsifiable monthly schedule (Critic B v3 `Phase-1 timeline realism`).** The 4–6 month estimate is a guess unless it is decomposed into checkable monthly checkpoints. v4 commits to the following month-by-month schedule for Phase 1, with the **exact PRs/issues that constitute "done"** at each milestone. Project tracking lives in `github.com/strata-dev/strata` issues with labels `phase-1`, `month-1`…`month-6`. **If Phase 1 is not done by month 6, the spec is wrong** — that is a falsifiable statement and we will say so in the v5 retrospective.

| Month | Theme | Concrete deliverables (each = labeled GitHub issue) | "Done" gate |
|---|---|---|---|
| **M1** | Skeleton | Issue `#P1-M1-1`: `stratad` boots, parses `config.toml`, drops privileges, idle-loops. Issue `#P1-M1-2`: `strata-core` skeleton + plugin host shell (loads no-op Wasm component). Issue `#P1-M1-3`: WIT contracts at `strata:plugin/lifecycle@0.1.0` published as crates 0.0.1. Issue `#P1-M1-4`: CI matrix green on macOS, Ubuntu-24.04, WSL2. Issue `#P1-M1-5`: `crates.io` placeholder reservations published from project key. | A no-op plugin loads, lifecycle hooks fire, daemon shuts down cleanly. CI is green. |
| **M2** | Manifest + signing | Issue `#P1-M2-1`: `strata-manifest` parses `strata.toml` per §4.4 schema (incl. `implied_tier`). Issue `#P1-M2-2`: `strata-signing` Ed25519 + cosign verification + Rekor lookup. Issue `#P1-M2-3`: `strata keys gen`, `strata trust add`. Issue `#P1-M2-4`: §0.1.1 cosign defense-in-depth verifier; `[trust] require_cosign` flag. Issue `#P1-M2-5`: name-collision audit committed (`docs/legal/name-audit.md`). | A signed plugin manifest is parsed, signature verified two ways, trust root pinned. Rejection vectors covered (bad sig, bad cosign, expired Rekor entry). |
| **M3** | Capability broker + powerbox | Issue `#P1-M3-1`: `strata-broker` capability handle issuance + revocation. Issue `#P1-M3-2`: `storage.local`, `system.clock`, `system.entropy` host implementations. Issue `#P1-M3-3`: `strata-host` Wasmtime embedding with `wasi:io@0.2`, `wasi:filesystem@0.2`. Issue `#P1-M3-4`: powerbox CLI prompt + `[Y]es/[n]o/[c]ustomize/[l]earn-more` flow per §4.6. Issue `#P1-M3-5`: tier-vs-capability check `tier_required = max(cap.min_tier)` property test. | A tier-1 plugin can read its sandboxed dir and cannot read `~/`. Property test passes 10k random manifests. Powerbox prompt rendered + accessible per §9.4.2. |
| **M4** | Distribution + install + reproducibility | Issue `#P1-M4-1`: `strata-oci` OCI fetch with sigstore signature verification. Issue `#P1-M4-2`: `strata-https-fetch` tarball path + detached `.sig`. Issue `#P1-M4-3`: `cargo-strata` (build/package/sign/publish/verify-rebuild). Issue `#P1-M4-4`: reproducible-packaging CI per §9.3 (`SOURCE_DATE_EPOCH`, deterministic tar). Issue `#P1-M4-5`: `strata install` + `strata pack` + `.stratapack` import. Issue `#P1-M4-6`: bridge-vector test suite scaffolding (deferred to Phase 2 use, scaffolded now). | `cargo strata package` produces byte-identical output across macOS + Linux for the hello-world fixture. `strata install` accepts both OCI and tarball paths. |
| **M5** | Operator DX + hello-world walkthrough + max_tier | Issue `#P1-M5-1`: `strata logs`, `strata diag`, `strata doctor`, `strata wrapper {enable,disable,repair,status}` per §8.3 wrapper UX. Issue `#P1-M5-2`: Prometheus `/metrics` endpoint + OpenTelemetry exporter. Issue `#P1-M5-3`: `[security] max_tier_allowed` per §9.4.1 incl. lowering workflow with prompts. Issue `#P1-M5-4`: `strata backup`/`restore`. Issue `#P1-M5-5`: hello-world walkthrough §9.3 CI-tested end-to-end on every commit. Issue `#P1-M5-6`: `strata init` wizard incl. screen-reader pass on Orca + VoiceOver. | Hello-world walkthrough green in CI on macOS + Linux + WSL2. `strata doctor` returns clean. `max_tier_allowed` lowering prompts an operator. |
| **M6** | Hardening + 1.0 release rehearsal | Issue `#P1-M6-1`: 2-of-3 Shamir release-key rehearsal per §12.1 (live key ceremony documented). Issue `#P1-M6-2`: Phase-1 acceptance criteria (§16) all green. Issue `#P1-M6-3`: documentation site `docs.strata.dev` + `strata-dev.github.io` published. Issue `#P1-M6-4`: `mirror.strata.dev` 5-year prepaid hosting contract signed (provenance receipt in `docs/governance/`). Issue `#P1-M6-5`: 1.0 RC published with cosign + Ed25519 sigs; security-disclosure SLA rehearsal. | All §16 acceptance tests pass on CI. RC artifacts verifiable by independent rebuild. Maintainer rotation review notes filed for months 1–6. |

**Slip policy.** If at the end of any month the listed deliverables are not green:
...
**Critical-path dependencies.** Wasmtime async maturity (M3), cargo-component stability (M4), and `cargo strata` plumbing (M4) are the highest-risk items. Each has a documented fallback in `docs/risk/phase-1-fallbacks.md`: e.g. if Component-Model async is not stable enough, M3 falls back to a sync-only host with documented latency cost; the fallback is a tracked GitHub project, not an unknown unknown.
...
|---|---|---|---|
| `core-runtime-lead` | `strata-core`, `strata-host`, lifecycle/broker bus, panic-fault isolation | `docs/maintainers/core-runtime-lead.md` | 2 |
| `mesh-lead` | `strata-plugin-mesh-{local,iroh,tailscale}`, chitchat, biscuit transport-bridging, transport phasing | `docs/maintainers/mesh-lead.md` | 2 |
| `agent-lead` | `strata-agent-host-*` adapters, MCP gateway, OS sandboxing of subprocess plugins, wrapper UX | `docs/maintainers/agent-lead.md` | 2 |
| `security-lead` | `strata-signing`, biscuit-auth, threat model, signing-key ceremony, security-disclosure intake | `docs/maintainers/security-lead.md` | 2 |
| `release-lead` | `cargo-strata`, reproducibility CI, mirror operations, deb/rpm/brew/winget release plumbing | `docs/maintainers/release-lead.md` | 2 |
...
**Maintainer roster (PR/CODEOWNERS-enforced).** Every L0/L1 crate (the trust footprint, plus `strata-bin` and `cargo-strata`) has **at least two named maintainers** in `crates/<crate>/MAINTAINERS.md`, drawn from the relevant role pool. A crate with only one maintainer cannot be merged into the L0/L1 layer.

| Layer | Crate | Primary role | Lead | Co-maintainer | Reviewer-of-record |
|---|---|---|---|---|---|
| L1 | `strata-core` | core-runtime-lead | A | B | C |
| L2 | `strata-broker` | core-runtime-lead | B | A | C |
| L2 | `strata-manifest` | release-lead | C | A | B |
| L2 | `strata-signing` | security-lead | A | C | B |
| L2 | `strata-oci`, `strata-https-fetch` | release-lead | B | C | A |
| L2 | `strata-host` | core-runtime-lead | A | B | C |
| L2 | `strata-ipc` | core-runtime-lead | C | B | A |
| L2 | `strata-bin`, `cargo-strata` | release-lead | C | A | B |
| L3 | `strata-plugin-mesh-*` | mesh-lead | (per crate) | (per crate) | (per crate) |
| L3 | `strata-agent-host-*` | agent-lead | (per crate) | (per crate) | (per crate) |
...
**Upstream OCI registry kill-switch.** Strata's canonical plugin registry is `ghcr.io/strata-dev/strata-plugins/...` (provided by GitHub Container Registry, owned by the strata-dev org per §0.1.1). If the project is wound down, abandoned, or its GitHub org goes dark, the kill-switch ensures users are not stranded:
...
| Directory listing (`/v2/_catalog`, `/v2/<plugin>/tags/list`) | **Mirror signs the listing** with the mirror's own Ed25519 key (`ed25519:strata-mirror-2026`). | Verifier confirms the listing came from the canonical mirror (so a network attacker cannot inject "this plugin doesn't exist"). The listing signature is for *enumeration integrity*, not artifact trust. |
...
**Implementation.** `strata-signing::verify_mirror_listing` validates the listing signature; `strata-signing::verify_artifact` is mirror-agnostic. The CI suite includes a `mirror-cdn-only.test` that asserts no path in the trust verifier consults the mirror's identity for artifact bytes.
...
| A | **A-N4-v3 (Minor)** | §8.3 wrapper-bypass UX trap | §8.3 — opt-in shell shim installed at `strata install`-time (zsh/bash/fish); warns when agent host is launched outside Strata session; `strata wrapper {enable,disable,repair,status}`; `strata doctor` validates wrapper presence + non-shadowing; honest note that the wrapper is a UX guardrail not a security boundary. |
...
| B | **B-reproducible-packaging** | `cargo strata package` byte-identical guarantee | §9.3 — `SOURCE_DATE_EPOCH` honored; deterministic tar (sorted entries, zeroed mtime, normalized mode/uid/gid); zstd level 19 single-threaded; `Cargo.lock` and `rust-toolchain.toml` mandatory; pinned WIT versions. CI matrix verifies bit-stability across macOS + Ubuntu-22.04 + Ubuntu-24.04. User-facing `cargo strata verify --rebuild` for third-party rebuild attestation. |
| B | **B-max_tier_allowed-lower-workflow** | Lowering ceiling with active above-ceiling plugins | §9.4.1 — lowering triggers an immediate scan; operator is **always** prompted with the affected-plugin list; default `unload-and-disable`; `cancel` reverts the edit; `strata security set-max-tier <N> --force-unload` for scripted use prints the affected list to stderr; never silent. `STRATA-W0214` / `STRATA-I0215` audit codes. |
| B | **B-strata-dev-org-provenance** | `strata-dev` org provenance | §0.1.1 — `strata-dev` GitHub org reserved at v4; owns the monorepo, Homebrew tap, docs site, and crates.io index for `strata-*`/`cargo-strata`. ≥3 maintainers with hardware MFA; org-policy doc at `docs/governance/org-policy.md`. Defense-in-depth: every release has BOTH Ed25519 (Shamir-2-of-3) AND sigstore/cosign signatures; daemon supports `[trust] require_cosign = true`. |
...
v3 closed every named objection. v4 closes every named minor and converts the two "you cannot prove this on paper" reserves into invariants the spec asserts and CI enforces (Phase-1 monthly schedule + `INV-BUS-1`). §16 acceptance criteria turn the spec into a contract — even before code exists, a critic can verify that every Phase-1 deliverable has a falsifiable test proposition. The expectation: 100/100. The remaining residual risks (Wasmtime async maturity, Iroh stability under Wi-Fi flap, biscuit revocation race windows) live in §13 and are the work of Phase 1 itself.
...
```
ATC-MAN-1
  GIVEN  a strata.toml with `tier = 1` and a capability whose `min_tier = 3`
  WHEN   strata-manifest::validate(manifest) is invoked
  THEN   it returns Err(Error::TierUnderDeclared { cap, min_tier: 3, declared: 1 })
  TEST   crates/strata-manifest/tests/tier_check.rs::tier_under_declared

ATC-MAN-2
  GIVEN  a strata.toml with `tier = 5` but only tier-1 capabilities declared
  WHEN   strata-manifest::validate(manifest) is invoked
  THEN   it returns Ok(_) — over-declaration is allowed; tier acts as a ceiling
  TEST   crates/strata-manifest/tests/tier_check.rs::tier_over_declared_ok

ATC-MAN-3
  GIVEN  a strata.toml with `runtime.kind = "wasm-component"` and a binary that is actually ELF
  WHEN   strata-host instantiates the plugin
  THEN   instantiation fails with Err(Error::RuntimeKindMismatch { declared, observed })
  TEST   crates/strata-host/tests/runtime_kind.rs::lie_at_instantiation_rejected

ATC-MAN-4 (Critic B S-NEW-2 / §9.4.1)
  GIVEN  a strata.toml with `tier = 3` AND a config with `[security] max_tier_allowed = 2`
  WHEN   strata install <manifest> is invoked
  THEN   it returns Err(Error::TierExceeded { plugin_tier: 3, ceiling: 2 })
  AND    no plugin state is mutated
  TEST   crates/strata-bin/tests/install_max_tier.rs::install_rejects_above_ceiling
```
...
ATC-PKG-1 (Critic B v3 B-reproducible-packaging)
  ...
  WHEN   `cargo strata package` runs twice in series in a clean workspace
...
ATC-PKG-2
  ...
  WHEN   `cargo strata package` runs on macOS-14 AND Ubuntu-22.04 AND Ubuntu-24.04 with the pinned toolchain
...
ATC-PKG-3
  ...
  WHEN   `cargo strata package` is invoked
  THEN   it returns exit code != 0 with message containing "Cargo.lock missing"
  TEST   crates/cargo-strata/tests/package.rs::no_lockfile_rejected

ATC-PKG-4
  GIVEN  a published .tar.zst + .sig + recorded source-commit
  WHEN   `cargo strata verify --rebuild` runs
  THEN   exit code == 0 AND output contains "PASS"
  TEST   crates/cargo-strata/tests/verify_rebuild.rs::happy_path
```
...
```
ATC-BRG-1
  GIVEN  a bridge biscuit with all 4 attenuation facts (dest_pin, rate_limit, ttl_le_3600s, bridge_marker)
  WHEN   strata-signing::verify_bridge(biscuit, verifier_node_id) runs
  THEN   it returns Ok(BridgeCap { ... })
  TEST   crates/strata-signing/tests/bridge_attenuation.rs::valid_bridge_accepted

ATC-BRG-2
  GIVEN  a bridge biscuit missing the rate_limit fact
  WHEN   strata-signing::verify_bridge(biscuit, verifier_node_id) runs
  THEN   it returns Err(STRATA-E0118)
  TEST   crates/strata-signing/tests/bridge_attenuation.rs::missing_rate_limit_rejected

ATC-BRG-3
  GIVEN  a bridge biscuit pinned to NodeId X but verified by NodeId Y (Y ≠ X)
  WHEN   strata-signing::verify_bridge(biscuit, Y) runs
  THEN   it returns Err(STRATA-E0119)
  TEST   crates/strata-signing/tests/bridge_attenuation.rs::wrong_dest_rejected

ATC-BRG-4
  GIVEN  a bridge biscuit whose TTL elapsed
  WHEN   strata-signing::verify_bridge(biscuit, verifier_node_id) at T_now > biscuit.ttl runs
  THEN   it returns Err(STRATA-E0120)
  TEST   crates/strata-signing/tests/bridge_attenuation.rs::expired_ttl_rejected
```
...
ATC-INT-1 (verifier-locality)
  ...
  TEST   crates/strata-plugin-scheduler/tests/integrity/no_worker_metric_execution.rs
...
ATC-INT-2 (custom-metric submitter-keyring)
  ...
  THEN   strata-plugin-scheduler::submit returns Err(MetricNotInKeyring)
  TEST   crates/strata-plugin-scheduler/tests/integrity/keyring_check.rs
...
ATC-INT-3 (policy immutability INV-INT-1)
  ...
  TEST   crates/strata-plugin-scheduler/tests/policy_immutability.rs::cannot_mutate_after_dispatch
...
ATC-INT-4 (streaming + Deterministic rejected at submit-time)
  ...
  TEST   crates/strata-plugin-scheduler/tests/submit_validation.rs::streaming_deterministic_rejected
```
...
ATC-STR-1 (credit-exhaustion stall)
  ...
  TEST   crates/strata-plugin-scheduler/tests/stream_stall.rs::credit_exhaustion_stalls_at_timeout
...
ATC-STR-2 (heartbeat ping)
  ...
  TEST   crates/strata-plugin-scheduler/tests/stream_heartbeat.rs::three_missed_pings_close_channel
...
ATC-STR-3 (chunk signing)
  ...
  TEST   crates/strata-plugin-scheduler/tests/stream_signing.rs::forged_chunk_rejected
```
...
```
ATC-WRP-1
  GIVEN  `strata install` has run with default settings AND user opts in to wrappers
  WHEN   the user runs `claude-code` in a fresh shell with STRATA_SESSION_ID unset
  THEN   stderr contains the string "claude-code launched outside Strata session"
...
ATC-WRP-2
  GIVEN  `strata wrapper disable` has run
...
ATC-MAX-1 (interactive prompt)
  ...
  WHEN   user lowers to max_tier_allowed = 3 via interactive `strata config reload`
...
ATC-MAX-2 (force-unload non-silent)
  ...
  WHEN   `strata security set-max-tier 3 --force-unload` runs
  THEN   stderr contains the affected-plugin list AND audit log has STRATA-W0214 entry per plugin
...
ATC-MAX-3 (cancel revert)
  ...
  AND    audit log has STRATA-I0216 entry
...
```
ATC-MIR-1
  GIVEN  a plugin .tar.zst served by mirror.strata.dev
  WHEN   strata install verifies it
  THEN   the verifier code path NEVER consults the mirror's identity for artifact bytes
  TEST   crates/strata-signing/tests/mirror_cdn_only.rs::no_mirror_in_artifact_trust

ATC-MIR-2
  GIVEN  a directory listing served by mirror with a valid mirror-listing signature
  WHEN   strata-oci::list_tags() is called
  THEN   it returns the listing AND records the listing-signature verification result
  TEST   crates/strata-oci/tests/listing.rs::mirror_listing_verified
```
...
ATC-REL-1 (cosign + Ed25519)
  ...
  WHEN   verifier runs `strata release verify <artifact>`
...
| Bridge cap-arithmetic | 4 | §6.4 |
| IntegrityPolicy | 4 | §7.5 |
...
Each acceptance test is a falsifiable proposition. The spec asserts: **Phase 1 is "done" iff all 31 propositions pass on CI.** No proposition relaxes; if a proposition cannot be implemented as written, the spec is wrong and the next revision documents the change.
