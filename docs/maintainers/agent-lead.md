...
2. Read the [Model Context Protocol spec](https://modelcontextprotocol.io/specification).
3. Walk the agent flow: install an MCP-compatible agent locally, run `entangle agent <name> --dry-run`, inspect the rewritten config, restore the original.
...
- Bundling a language runtime (Node, Python) with the daemon — explicit non-goal in the spec; reversal needs project-wide RFC.
